...
Return:
     None
**************************************************/
void Carrier_Test(uint8_t b_enable)
{
	uint8_t j;
	uint8_t WriteArr[4];
...
//write REG4
	SPI_Bank1_Write_Reg((WRITE_REG_CMD|4),WriteArr);
...
Return:
     None
**************************************************/
void BER_Test(uint16_t ms,uint32_t* received_total_bits,uint32_t* received_error_bits)
{
	uint8_t WriteArr[5];
...
////////////////////////////////////////////////////////
	...
	SPI_Write_Buf((WRITE_REG_CMD|10),WriteArr,5);//change RX0 address for BER test
...
////////////////////////////////////////////////////////
	...
	SPI_Bank1_Write_Reg((WRITE_REG_CMD|12),&(WriteArr[0]));
...
//restore RX0 address
	SPI_Write_Buf((WRITE_REG_CMD|10),(uint8_t*)RX0_Address,5);//restore RX0 address
...
//write REG4
	SPI_Bank1_Write_Reg(WRITE_REG_CMD|0x04,WriteArr);
}
...
//write REG5
	SPI_Bank1_Write_Reg(WRITE_REG_CMD|0x05,WriteArr);
}
...
void PN9_TX(uint8_t b_enable)
{
	...
	if (b_enable)//start send PN9
	{
		...
		WriteArr[2] = 0x00;
		WriteArr[3] = 0x00;
		SPI_Bank1_Write_Reg(WRITE_REG_CMD|0x01,WriteArr); //write bank1_reg1
		
		WriteArr[0] = 0xA0;
		WriteArr[1] = 0xFC;	
		WriteArr[2] = 0x8C;
		WriteArr[3] = 0x02;
		SPI_Bank1_Write_Reg(WRITE_REG_CMD|0x02,WriteArr); //write bank1_reg2
		
		WriteArr[0] = 0x10;
		WriteArr[1] = 0x12;	
		WriteArr[2] = 0x73;
		WriteArr[3] = 0x00;
		SPI_Bank1_Write_Reg(WRITE_REG_CMD|0x0C,WriteArr); //write bank1_reg12
	}
	else//stop send PN9
	{
		WriteArr[0] = 0xC0;
